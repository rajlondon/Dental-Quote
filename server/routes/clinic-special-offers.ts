...
import { eq, and, inArray, gte, or, isNull } from 'drizzle-orm';
import { z } from 'zod';
    ...
    const offers = await db
      ...
      .where(
        and(
          ...
          // Only return offers that are still valid (or have no end date)
          or(
            gte(specialOffers.validUntil, new Date()),
            eq(specialOffers.validUntil, null)
          )
        )
      );
...
router.post('/match-treatments', async (req, res) => {
  try {
    ...
    const matchedOffers = offers.map(offer => {
