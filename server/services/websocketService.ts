...
export class WebSocketService {
  ...
  // Synchronize booking update across portals
  syncBookingUpdate(bookingId: number, updateData: any) {
  ...
  // Synchronize quote update across portals
  syncQuoteUpdate(quoteId: number, updateData: any) {
    this.clients.forEach((client, userId) => {
      if (client.ws.readyState === WebSocket.OPEN) {
        this.sendToUser(userId, {
          type: 'quote_update',
  ...
  syncAppointmentUpdate(appointmentId: number, updateData: any) {
    this.clients.forEach((client, userId) => {
      if (client.ws.readyState === WebSocket.OPEN) {
        this.sendToUser(userId, {
