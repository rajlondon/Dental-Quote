...
import TroubleShootedOfferCard, { OfferData } from './TroubleShootedOfferCard';
import { apiRequest } from '@/lib/queryClient';
import { useLocation } from 'wouter';

console.log('COMPONENT_ID: FixedHomePageOffers LOADED - ' + new Date().toISOString());

interface HomePageOffersProps {
  className?: string;
}

// This component works directly with the actual API data structure
const FixedHomePageOffers: React.FC<HomePageOffersProps> = ({ className = '' }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [, setLocation] = useLocation();
  const [autoPlay, setAutoPlay] = useState(true);
  const [isHovering, setIsHovering] = useState(false);

  // Fetch special offers from backend API
  const { data: offers = [] as OfferData[], isLoading, error } = useQuery<OfferData[]>({
  ...
  useEffect(() => {
    if (!autoPlay || isHovering || !offers || offers.length === 0) return;
  ...
  // Debug: Log offers when they change
  useEffect(() => {
    console.log('FixedHomePageOffers - Received offers data:', offers);
  }, [offers]);
  ...
  if (error || !offers || offers.length === 0) {
    console.log('No offers to display or error:', error);
    return null; // Don't show anything if no offers
  }

  const visibleOffers: OfferData[] = [];
  ...
  return (
    <section 
      ...
      id="fixed-special-offers-container"
    >
      <div className="container mx-auto px-4">
        ...
        <div style={{
          ...
          FIXED COMPONENT ({new Date().toLocaleTimeString()}) - {offers.length} Offers
        </div>
        ...
        {offers.length > 1 && (
          <div className="flex justify-center mt-6 md:hidden">
            {Array.from({ length: offers.length }).map((_, index) => (
              <Button 
                key={index}
                variant="ghost" 
                size="icon" 
...
export default FixedHomePageOffers;