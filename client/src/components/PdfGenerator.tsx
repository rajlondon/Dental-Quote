...
export const generateQuotePdf = ({
  ...
  doc.text("Your Personalized Smile Journey", pageWidth / 2, 25, {
    align: "center",
  });
  ...
  if (items.length > 0) {
    treatmentSummary = items
      .map((item) => `${item.quantity} ${item.treatment}`)
      .join(" + ");
  } else {
  ...
  items.forEach((item, index) => {
    ...
    doc.text(
      truncateText(formatTreatmentName(item.treatment), 40),
      colPositions[0].x + 5,
      yPos,
    );
    ...
    doc.text(
      ...
      colPositions[1].x + colPositions[1].width / 2,
      yPos,
      { align: "center" },
    );
    ...
    doc.text(
      ...
      colPositions[2].x + colPositions[2].width / 2,
      yPos,
      { align: "center" },
    );
    ...
    doc.text(
      ...
      colPositions[3].x + colPositions[3].width / 2,
      yPos,
      { align: "center" },
    );
    ...
    doc.text(
      item.guarantee || "N/A",
      colPositions[4].x + colPositions[4].width / 2,
      yPos,
      { align: "center" },
    );
  ...
  if (flightEstimate) {
    ...
    doc.setFontSize(10);
    doc.text(
      `Return flight from ${departureCity} (${travelMonth})`,
      margin + 5,
      yPos,
    );
  ...
  const grandTotalUSD = flightEstimate
    ? totalUSD + Math.round(flightEstimate * 1.3)
    : totalUSD;
  ...
  if (dentalChart && dentalChart.length > 0) {
    ...
    if (selectedTeeth.length > 0) {
      ...
      const upperTeeth = selectedTeeth.filter(
        (tooth) => tooth.section === "Upper",
      );
      const lowerTeeth = selectedTeeth.filter(
        (tooth) => tooth.section === "Lower",
      );
      ...
      if (upperTeeth.length > 0) {
        ...
        upperTeeth.forEach((tooth) => {
          ...
          doc.text(
            `Tooth ${tooth.id} (${tooth.name}): ${treatments}`,
            margin + 20,
            yPos,
          );
          yPos += 6;
      ...
      doc.text(
        "No specific teeth have been selected for treatment.",
        margin + 5,
        yPos,
      );
      doc.text(
        "Your dental plan may include general procedures that apply to all teeth,",
        margin + 5,
        yPos + 8,
      );
      doc.text(
        "or your dentist will determine the specific teeth during your consultation.",
        margin + 5,
        yPos + 16,
      );
    }
  ...
  if (clinics && clinics.length > 0) {
    ...
    clinics.forEach((clinic, index) => {
      ...
      doc.text(
        `£${clinic.priceGBP.toLocaleString()}`,
        clinicColPos[1].x + clinicColPos[1].width / 2,
        yPos,
        { align: "center" },
      );
      ...
      if (extrasArray.includes("Translator")) {
        ...
        if (extrasArray.includes("Hotel") && extrasArray.includes("Transfer")) {
          ...
          extrasArray.includes("Hotel") ||
          extrasArray.includes("Transfer")
        ) {
          doc.addImage(
            TRANSLATOR_ICON_BASE64,
            "PNG",
  ...
  doc.text(
    `Save up to ${totalGBP > 0 ? Math.round(100 - (totalGBP / (totalGBP * 3)) * 100) : 65}% compared to UK dental prices`,
    margin + 15,
    yPos + 5,
  );
  ...
  doc.addImage(TICK_ICON_BASE64, "PNG", margin + 5, yPos + 1, 4, 4);
  doc.text(
    "Comprehensive treatment plan with quality guarantees",
    margin + 15,
    yPos + 5,
  );
  ...
  doc.addImage(TICK_ICON_BASE64, "PNG", margin + 5, yPos + 1, 4, 4);
  doc.text(
    "All-inclusive package with accommodation and transfers available",
    margin + 15,
    yPos + 5,
  );
  ...
  doc.text(
    "1. Review your treatment plan and clinic options",
    margin + 5,
    yPos,
  );
  yPos += 8;
  doc.text(
    "2. Login to your Patient Portal to approve your treatment plan",
    margin + 5,
    yPos,
  );
  yPos += 8;
  doc.text(
    "3. Secure your booking with a £200 refundable deposit",
    margin + 5,
    yPos,
  );
  yPos += 8;
  doc.text(
    "4. Our team will contact you to schedule your consultation and travel dates",
    margin + 5,
    yPos,
  );
  ...
  doc.text(
    `Generated for ${patientName || "Valued Patient"} on ${formattedDate} | Quote ID: MDF-${datePart}-${Math.floor(Math.random() * 1000)}`,
    pageWidth / 2,
    footerYPos,
    { align: "center" },
  );
