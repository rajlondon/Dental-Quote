...
// Unicode symbols for icons - these work better in PDF than text in brackets
const TICK_ICON = "✓";          // Checkmark for list items
const HOTEL_ICON = "🏨";        // Hotel icon (Unicode hotel emoji)
const TRANSFER_ICON = "🚕";     // Transfer/taxi icon (Unicode taxi emoji)
const TRANSLATOR_ICON = "🗣️";   // Translator/specialist icon (Unicode speaking emoji)
...
const generateQuotePdf = ({
  ...
  if (clinics && clinics.length > 0) {
    ...
    clinics.forEach((clinic, index) => {
      ...
      if (clinic.extras.includes('Hotel')) {
        extrasText += `${HOTEL_ICON} Hotel `;
      }
      if (clinic.extras.includes('Transfer')) {
        extrasText += extrasText ? `· ${TRANSFER_ICON} Transfer ` : `${TRANSFER_ICON} Transfer `;
      }
      if (clinic.extras.includes('Translator')) {
        extrasText += extrasText ? `· ${TRANSLATOR_ICON} Translator` : `${TRANSLATOR_ICON} Translator`;
      }
  ...
  // Add checklist items with checkmark symbols - matching HTML template format
  // Using our checkmark icon
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.setTextColor(0, 0, 0);
  
  // First item with spacing that matches HTML
  doc.text(`${TICK_ICON} Vetted, Trusted Clinics`, margin + 5, yPos + 5);
  
  // Second item
  yPos += 10;
  doc.text(`${TICK_ICON} Concierge Support from Start to Finish`, margin + 5, yPos + 5);
  
  // Third item
  yPos += 10;
  doc.text(`${TICK_ICON} Safe Payment & Transparent Pricing`, margin + 5, yPos + 5);
  
  // Fourth item
  yPos += 10;
  doc.text(`${TICK_ICON} Enjoy Istanbul While Enhancing Your Smile`, margin + 5, yPos + 5);
