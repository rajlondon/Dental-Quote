...
const ClinicGuard: React.FC<ClinicGuardProps> = ({ children }) => {
  ...
  if (!user) {
    console.log('ClinicGuard: No user found, redirecting to login');
    return <Redirect to="/portal-login" />;
  }

  console.log('ClinicGuard: User found:', { id: user.id, email: user.email, role: user.role });

  if (user.role !== 'clinic' && user.role !== 'admin') {
    console.log('ClinicGuard: User role not authorized:', user.role);
    toast({
      title: 'Access Denied',
      description: `You do not have permission to access the clinic portal. Your role: ${user.role}`,
      variant: 'destructive',
    });
    return <Redirect to="/portal-login" />;
  }

  console.log('ClinicGuard: Access granted for user role:', user.role);
