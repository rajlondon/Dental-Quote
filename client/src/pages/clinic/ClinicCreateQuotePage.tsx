...
export default function ClinicCreateQuotePage() {
  const [, setLocation] = useLocation();
  const params = useParams();
  const quoteId = params?.id ? parseInt(params.id) : undefined;
  
  const { getQuoteQuery, createQuoteVersionMutation, updateQuoteMutation } = useQuotes();
  ...
  const handleSubmit = async (quoteData: QuoteData, sendToPatient: boolean) => {
    // If quote is in "assigned" status, update it to "in_progress"
    if (quoteRequest.status === "assigned") {
    ...
    <div className="container mx-auto py-6 px-4">
      ...
      <div className="mt-6">
        <QuoteForm
          ...
          isSubmitting={createQuoteVersionMutation.isPending || updateQuoteMutation.isPending}
        />
      </div>
    </div>
  );
